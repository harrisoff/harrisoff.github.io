window.onload = () => {
  ...
  document.getElementById("img-upload").addEventListener("click", async () => {
    ...
    try {
      // original image
      // upload & share
      let origin_share_url = await uploadAndShare(
        img_origin,
        dir_name,
        origin_name
      );
      ...
      document
        .getElementById("origin-img")
        .setAttribute("src", origin_share_url);
      document.getElementById("img-url").value = origin_share_url;

      // generate preview image
      if (if_preview) {
        let blob =
          img_origin.type.indexOf("gif") !== -1
        ...
        // upload & share
        let preview_share_url = await uploadAndShare(
          img_compressed,
          dir_name,
          preview_name
        );
        ...
        document
          .getElementById("pre-img")
          .setAttribute("src", preview_share_url);
...
async function uploadAndShare(blob, dir_name, file_name) {
  if (blob.size >= mb_4) {
    ...
    // upload
    let upload_response = await uploadPic60Step2(
      blob,
      upload_url_response,
      blob.size
    );
...
function file_names(file_name, file_type) {
  ...
  return [
    ...
    `${file_name}-preiview-${timestamp}.${file_type}`
  ];
}
